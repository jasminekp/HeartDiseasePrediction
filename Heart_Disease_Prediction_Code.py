...
"""
There are a Unicode character '\u0332', COMBINING LOW LINE*, which acts as an underline on the character that precedes it in a string. The center() method will center align the string, using a specified character (space is the default) as the fill character.
"""
...
def exploratory_data_analysis(df):
    ...
    print("\nCorrelation Between Various Features - It is always better to check the correlation between the features so that we can analyse which feature is negatively correlated and which is positively correlated.")
    ...
    print("\nCorrelation with the AHD (Target) feature - Except for the Chest Pain and Maximum Heart Rate Achieved features, all others are negatively correlated with the AHD (Target) feature.")
...
def Sex_feature_analysis(df):
    ...
    print("\nRatio of Sex - We observe that female to male ratio is approximate 1:2.")
    
    plt.figure(figsize=(18,9)) # Create a new figure, or activate an existing figure
    sns.set_context('notebook',font_scale = 1.5) # Set the parameters that control the scaling of plot elements
  #  sns.countplot(df['Sex'],hue=df["Slope"]) # Show the counts of observations in each categorical bin using bars
    sns.countplot(x=df['Sex'],hue=df["Slope"]) # Show the counts of observations in each categorical bin using bars
...
def ChestPain_feature_analysis(df):
    ...
    plt.title("Types of ChestPain - 0 (asymptomatic), 1 (nonanginal), 2 (nontypical) and 3 (typical)")
    plt.tight_layout(); plt.grid(True); plt.show()
    
    plt.figure(figsize=(18,9)) # Create a new figure, or activate an existing figure
    sns.set_context('notebook',font_scale = 1.5) # Set the parameters that control the scaling of plot elements
    sns.countplot(x=df['ChestPain'],hue=df["AHD"]) # Show the counts of observations in each categorical bin using bars
...
def MaxHR_feature_analysis(df):
    ...
    plt.scatter(df.Age[df.AHD==1], df.MaxHR[df.AHD==1], c="salmon") # Scatter with positive examples
...
def k_nearest_neighbor(df):
    ...
    # Generate plot:-    
    plt.style.use('seaborn-v0_8'); plt.figure()
...
def auc_roc_curve(df):
    ...
    print("\nAUC Scores-")
    print("(i) K-Nearest Neighbor (KNN): ", auc_score1*100)
    print("(ii) Naive Bayes Classifier: ", auc_score2*100)
    print("(iii) Support Vector Machine (SVM): ", auc_score3*100)
    print("(iv) Decision Tree Using Gini Index: ", auc_score4*100)
    print("(v) Decision Tree Using Entropy: ", auc_score5*100)
...
def main():
    ...
    print("5. Chol:- Serum Cholesterol Measurement (mg/dl)")
    ...
    print("2. Resting Blood Pressure (RestBP):- Anything between 120-140 (mm Hg on admission to the hospital) is typically a cause for concern.")
    print("3. Serum Cholesterol Measurement (Chol):- Anything between 200-300 (mg/dl) is typically a cause for concern.")
...
# Call the main function; Execution starts here.
